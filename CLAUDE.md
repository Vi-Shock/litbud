...
| 2026-04-17 | Crash on "Read Again" — `startRecording() called on uninitialized AudioRecord` | `AudioRecord` constructor returns `STATE_UNINITIALIZED` when mic hardware not yet released from previous session | Check `recorder.state != STATE_INITIALIZED` before `startRecording()`; release and return empty bytes — `LitBudScreen.kt` |
| 2026-04-17 | Crash on "Read Again" — `LiteRtLmJniException: miniaudio error -10` | JNI exception from native decoder propagates past `onError` callback, crashes coroutine | Wrap `runInference(audioClips)` in try-catch in `processReading()` — `LitBudViewModel.kt` |
...
| 2026-04-17 | App crash on first attempt — `Conversation is not alive` | `resetConversation(systemInstruction=...)` passes a new system instruction to `ConversationConfig`; on an already-initialized engine this causes `engine.createConversation()` to fail silently (Gallery catches exception), leaving `instance.conversation` as the just-closed old one. Next `runInference` hits `checkIsAlive` → crash | Never pass `systemInstruction` to `resetConversation`. Engine keeps the system instruction from `initialize()`. Removed `coachingSystemInstruction` entirely — `LitBudViewModel.kt` |
| 2026-04-17 | Three duplicate model instances in picker | `task.models` is a singleton `mutableListOf()` (Hilt); `loadModelAllowlist()` can run multiple times (allowlist refresh), re-adding the same model each run | Use a deduplicating `AbstractMutableList` for `models` in `LitBudTask.kt` |
